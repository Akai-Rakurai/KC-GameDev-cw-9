...
public class CharacterController2D : MonoBehaviour
{
	...
	void fire()
    {
		if(m_FacingRight && Input.GetMouseButtonDown(1))
        {
			BulletClone = Instantiate(Bullet, Spawn.position, Spawn.rotation);
			BulletClone.GetComponent<Rigidbody2D>().velocity = transform.right * Speed;
			Destroy(BulletClone, 2f);
        }
		else if(!m_FacingRight && Input.GetMouseButtonDown(1))
        {
			BulletClone = Instantiate(Bullet, Spawn.position, Spawn.rotation);
			BulletClone.GetComponent<Rigidbody2D>().velocity = transform.right * -Speed;
			Destroy(BulletClone, 2f);
